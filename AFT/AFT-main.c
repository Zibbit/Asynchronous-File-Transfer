                   ...
                   gpointer      user_data)
{
  ...
  if (! evd_transport_open_finish (EVD_TRANSPORT (obj),
                                   ...
                                   &error))
    {
      g_printerr ("Error connecting to AFT server: %s\n", error->message);
      g_error_free (error);
                              ...
                              gpointer      user_data)
{
  ...
  if (! AFT_protocol_register_sources_finish (AFT_PROTOCOL (obj),
                                                  res,
                                                  &sources,
                                                  &error))
    {
      ...
      while (node != NULL)
        {
          ...
          error = AFT_source_get_error (source);
          if (error != NULL)
            {
...
protocol_register_sources (void)
{
  ...
  AFT_protocol_register_sources (protocol,
                                   AFT_source_get_peer (shared_file.source),
                                   sources,
                                   NULL,
                            ...
                            gpointer      user_data)
{
  ...
  if (size < 0)
  ...
  else if (size > 0)
    {
      ...
      bytes_left = AFT_source_get_size (push_req->shared_file->source) - push_req->total_sent;
                     ...
                     gpointer      user_data)
{
  ...
  AFT_source_set_size (shared_file.source,
...
static void
protocol_seeder_push_request (AFTProtocol *protocol,
                              GAsyncResult    *result,
                              ...
                              gpointer         user_data)
{
  ...
  g_assert_cmpint (g_strcmp0 (AFT_source_get_id (shared_file.source),
                              source_id),
                   ==, 0);
...
transport_on_new_peer (EvdTransport *transport,
                       ...
                       gpointer      user_data)
{
  AFT_source_set_peer (shared_file.source, _peer);
...
main (gint argc, gchar *argv[])
{
  ...
  g_free (file_name);
  shared_file.source = AFT_source_new (NULL,
                                           base_name,
                                           file_type,
                                           file_size,
                                           AFT_SOURCE_FLAGS_CHUNKABLE |
                                           AFT_SOURCE_FLAGS_PUBLIC,
                                           NULL);
  g_free (base_name);
  g_object_unref (file_info);
  ...
  protocol = AFT_protocol_new (&vtable,
                                   NULL,
                                   NULL);
  rpc = AFT_protocol_get_rpc (protocol);
