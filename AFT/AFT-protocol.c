...
G_DEFINE_TYPE (AFTProtocol, AFT_protocol, G_TYPE_OBJECT)

#define AFT_PROTOCOL_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), \
                                           AFT_TYPE_PROTOCOL, \
                                           AFTProtocolPrivate))
...
static void     AFT_protocol_class_init         (AFTProtocolClass *class);
static void     AFT_protocol_init               (AFTProtocol *self);

static void     AFT_protocol_finalize           (GObject *obj);
static void     AFT_protocol_dispose            (GObject *obj);
...
static void
AFT_protocol_class_init (AFTProtocolClass *class)
{
  GObjectClass *obj_class = G_OBJECT_CLASS (class);

  obj_class->dispose = AFT_protocol_dispose;
  obj_class->finalize = AFT_protocol_finalize;

  g_type_class_add_private (obj_class, sizeof (AFTProtocolPrivate));
}

static void
AFT_protocol_init (AFTProtocol *self)
{
  AFTProtocolPrivate *priv;

  priv = AFT_PROTOCOL_GET_PRIVATE (self);
  self->priv = priv;
...
static void
AFT_protocol_dispose (GObject *obj)
{
  AFTProtocol *self = AFT_PROTOCOL (obj);
  ...
  G_OBJECT_CLASS (AFT_protocol_parent_class)->dispose (obj);
}

static void
AFT_protocol_finalize (GObject *obj)
{
  AFTProtocol *self = AFT_PROTOCOL (obj);

  g_object_unref (self->priv->rpc);

  G_OBJECT_CLASS (AFT_protocol_parent_class)->finalize (obj);
}
...
typedef struct
{
  AsyncOpData *data;
  guint op_index;
} AFTProtocolOperation;

static void
op_register_content (AFTProtocol *self,
                     JsonNode        *params,
                     guint            invocation_id,
                     gpointer         context)
{
  ...
  for (i=0; i<json_array_get_length (a); i++)
    {
      ...
      gint64 num;
      gsize size = 0;
      guint flags = AFT_SOURCE_FLAGS_NONE;
      gchar **tags = NULL;
      AFTSource *source = NULL;
      ...
      /* create source */
      source = AFT_source_new (EVD_PEER (context),
                                   name,
                                   type,
                                   size,
                                   flags,
      ...
      if (json_object_has_member (obj, "id") &&
          json_object_has_member (obj, "signature"))
        {
          ...
          if (_id != NULL && _signature != NULL)
            {
              AFT_source_set_id (source, _id);
              AFT_source_set_signature (source, _signature);
            }
        }
      ...
      json_object_set_string_member (reg_node_obj,
                                     "signature",
                                     AFT_source_get_signature (source));
...
static void
op_unregister_content (AFTProtocol *self,
                       JsonNode        *params,
                       guint            invocation_id,
                       gpointer         context)
{
...
rpc_on_method_called (EvdJsonrpc  *jsonrpc,
                      ...
                      gpointer     user_data)
{
  AFTProtocol *self = AFT_PROTOCOL (user_data);
...
static void
op_seeder_push_request (AFTProtocol *self,
                        JsonNode        *params,
                        guint            invocation_id,
                        gpointer         context)
{
...
static JsonNode *
AFT_source_to_json (AFTSource *source)
{
  JsonNode *node;
  JsonObject *obj;
  const gchar **tags;
  ...
  json_object_set_string_member (obj,
                                 "name",
                                 AFT_source_get_name (source));
  json_object_set_string_member (obj,
                                 "type",
                                 AFT_source_get_content_type (source));
  json_object_set_int_member (obj, "size", AFT_source_get_size (source));
  json_object_set_int_member (obj, "flags", AFT_source_get_flags (source));

  if (AFT_source_get_id (source) != NULL)
    json_object_set_string_member (obj, "id", AFT_source_get_id (source));

  if (AFT_source_get_signature (source) != NULL)
    json_object_set_string_member (obj,
                                   "signature",
                                   AFT_source_get_signature (source));
      ...
      for (i=0; i<json_array_get_length (arr); i++)
        {
          ...
          source = AFT_SOURCE (node->data);

          if (json_node_is_null (err_member))
            {
              AFT_source_set_id (source,
                                     json_object_get_string_member (obj, "id"));
              AFT_source_set_signature (source,
                              json_object_get_string_member (obj, "signature"));

            }
          else
            {
              AFT_source_take_error (source,
...
AFTProtocol *
AFT_protocol_new (AFTProtocolVTable *vtable,
                      gpointer               user_data,
                      GDestroyNotify         user_data_free_func)
{
  AFTProtocol *self;
  ...
  self = g_object_new (AFT_TYPE_PROTOCOL, NULL);
...
EvdJsonrpc *
AFT_protocol_get_rpc (AFTProtocol *self)
{
  g_return_val_if_fail (AFT_IS_PROTOCOL (self), NULL);

  return self->priv->rpc;
}

gboolean
AFT_protocol_handle_content_request (AFTProtocol    *self,
                                         AFTSource      *source,
                                         ...
                                         GError            **error)
{
  ...
  g_return_val_if_fail (AFT_IS_PROTOCOL (self), FALSE);
  g_return_val_if_fail (AFT_IS_SOURCE (source), FALSE);
...
gboolean
AFT_protocol_handle_content_push (AFTProtocol    *self,
...
{
  g_return_val_if_fail (AFT_IS_PROTOCOL (self), FALSE);
  g_return_val_if_fail (AFT_IS_TRANSFER (transfer), FALSE);
  g_return_val_if_fail (EVD_IS_WEB_SERVICE (web_service), FALSE);
  g_return_val_if_fail (EVD_IS_HTTP_CONNECTION (conn), FALSE);
  g_return_val_if_fail (EVD_IS_HTTP_REQUEST (request), FALSE);
...
gboolean
AFT_protocol_request_content (AFTProtocol  *self,
                                  EvdPeer          *peer,
                                  const gchar      *source_id,
                                  const gchar      *transfer_id,
                                  gboolean          is_chunked,
...
void
AFT_protocol_register_sources (AFTProtocol     *self,
                                   EvdPeer             *peer,
                                   GList               *sources,
                                   ...
                                   gpointer             user_data)
{
  ...
  g_return_if_fail (AFT_IS_PROTOCOL (self));
  ...
  while (node != NULL)
    {
      ...
      g_assert (AFT_IS_SOURCE (node->data));
      source = AFT_SOURCE (node->data);

      source_json = AFT_source_to_json (source);
...
gboolean
AFT_protocol_register_sources_finish (AFTProtocol  *self,
                                          GAsyncResult     *result,
                                          GList           **sources,
                                          GError          **error)
{
